...
| 5 — soft delete keeps past orders | the snapshot columns, and `15_online_orders_rls.test.sql` |
| 6 — sorting and search | `src/lib/online/catalogue.test.ts` |
| 7 — staff cannot open owner pages | `15_online_orders_rls.test.sql`, as five different people, plus `requireOwnerOrAdmin` on the screen |
